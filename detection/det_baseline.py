...
def calculate_dataset_statistics(dataset, num_classes):
    ...
    for data in dataset:
        shape_counter.append(data['shape'])  # h, w
        counts = np.unique(data['labels'], return_counts=True)
        bbox.extend(data['bboxes'])
    ...
    return pixel_counter, shape_counter, bbox, num_bbox
...
def calculate_recommended_params(dataset, num_classes, arch, max_iter=None):
    ...
    pixel_counter, shape_counter, bbox, num_bbox = calculate_dataset_statistics(
        dataset, num_classes)
    ...
    # the maximize size for experiments is set to 2 * origin image size
    ...
    recommanded_params['size_bs_iter'] = []
    for img_h, img_w in zip(img_h_list, img_w_list):
        ...
        recommanded_params['size_bs_iter'].append({
...
def gen_cfg_file(cfg_output_path, backbone, num_classes, label_map, metric_type,
    ...
    batch_size = recommanded_params['size_bs_iter'][0]['batch_size']
    eval_size = recommanded_params['size_bs_iter'][0]['eval_size']
    input_size = recommanded_params['size_bs_iter'][0]['input_size']
    max_iter = recommanded_params['size_bs_iter'][0]['max-iter']
    ...
    cfg_file = cfg_template.render(num_classes=num_classes,
                                   backbone=backbone,
                                   size_bs_iter='\'$' + json.dumps(recommanded_params['size_bs_iter'][0]) + '\'',
                                   num_bbox=str(recommanded_params['num_bbox'] * 2),
                                   label_map=label_map,
                                   eval_size=eval_size,
                                   max_iter=max_iter,
...
def gen_hpo_file(
    ...
    if metric_type == 'p/r':
        ...
        for val_name in val_names:
            metrics.append({
                'data_name': val_name,
                'metric_type': 'mAPEvaluator/Precision/mAP',
...
class DetBaseline(BaselineBase):
    ...
    def gen_cfgs(self, baseline_yaml_path, hpo_path, rerun_best_trial=False):
        ...
        recommanded_params = calculate_recommended_params(dataset, len(label_map), arch=self.kwargs.get('arch'),
        ...
        gen_hpo_file(
            ...
            size_bs_iter=recommanded_params['size_bs_iter'],
            val_names=val_names,
...
@BASELINE.register_module()
class LocalDetBaseline(BaselineBase):
    DEFAULT_CONFIG = BaselineDefaults.DetBaseline_cfg
