...
from .templates import cfg_template, cfg_template_fast
from .templates import local_hpo_template, cluster_hpo_template
from .templates import local_hpo_template_fast, cluster_hpo_template_fast # noqa
from .templates import local_hpo_template_cost_down, cluster_hpo_template_cost_down # noqa
...
def calculate_recommanded_params(dataset, num_classes, preset):
    ...
    num_iter += (num_classes - 2) // 2 * 1000
    num_iter = min(num_iter, preset.MAX_ITER[level])
...
class SegBaseline(BaselineBase):
    DEFAULT_CONFIG = BaselineDefaults.SegBaseline_cfg

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.kwargs = merge_dict(BaselineDefaults.SegBaseline_cfg, self.kwargs)
...
@BASELINE.register_module()
class LocalSegBaseline(BaselineBase):
    DEFAULT_CONFIG = BaselineDefaults.LocalSegBaseline_cfg

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.kwargs = merge_dict(
            BaselineDefaults.LocalSegBaseline_cfg, self.kwargs)
        self.cfg_template = cfg_template
        self.local_hpo_template = local_hpo_template
    ...
    def gen_cfgs(self, baseline_yaml_path, hpo_path, rerun_best_trial=False):
        ...
        # gen exp.yaml
        exp_generator = EXPGenerator(cfg_template, baseline_yaml_path)
        exp_generator.get_params(recommended_params)
        exp_generator.dump(cfg_output_path)

        # gen hpo.yaml
        gen_hpo_file(hpo_output_path=hpo_output_path,
                     hpo_template=self.local_hpo_template,
                     base_exp_path=cfg_output_path,
                     val_names=val_names,
                     lr=lr,
                     max_trials=self.kwargs.get('max_trials'),
...
@BASELINE.register_module()
class FastSegBaseline(BaselineBase):
    DEFAULT_CONFIG = BaselineDefaults.FastSegBaseline_cfg

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.kwargs = merge_dict(BaselineDefaults.FastSegBaseline_cfg, self.kwargs)
        self.cfg_template = cfg_template_fast
        self.cluster_hpo_template = cluster_hpo_template_fast
    ...
    def gen_cfgs(self, baseline_yaml_path, hpo_path, rerun_best_trial=False):
        ...
        lr_small *= self.kwargs.get('gpu_per_trial')
        lr_small /= 2.0
        lr = [lr_small, lr_small * 3, lr_small * 9]

        # other params
        recommended_params['lr'] = lr_small * 3
        recommended_params['num_classes'] = len(label_map)
        recommended_params['label_map'] = label_map
        pretrained_weight = self.kwargs.get('pretrained_weight')
        if pretrained_weight:
...
class SegBaseline2(SegBaseline):
    """ SegBaseline 更新版, 使用open-box并增加早停器以提速, 其余与SegBaseline保持一致 """
    DEFAULT_CONFIG = BaselineDefaults.SegBaseline_cfg
    ...
    def __init__(self, **kwargs):
        ...
        self.kwargs = merge_dict(BaselineDefaults.SegBaseline_cfg, self.kwargs)
        self.cfg_template = cfg_template
        self.cluster_hpo_template = cluster_hpo_template_cost_down
